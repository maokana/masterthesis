...
├── interaction.py --上記SQL出力結果を基に分野間での交流回数をカウントするコード　※リサーチトピックの数の分野間の違いを考慮する。
└── word_regression_clustering.py -- 分野間の分析語のcos類似度を左辺に、各変数（リサーチトピックの一致度、分野の一致度、交流メンバーの一致度、分野間交流度）の重みを計算し、それを基に単語をクラスタリングするコード
...
分析対象語の分野間のcos類似度を左辺とし、リサーチトピック/分野説明の類似度、リサーチトピック参加メンバーの類似度、リサーチトピックへの相互参加回数等を変数として各変数の重みづけを算出し、その値を基に分析対象語のクラスタリング（分類）を行う。

### 実行例

```bash
python word_regression_clustering.py --input similartiy_sum.csv --output word_clustering.csv
```

### 内容

- 単語ごとに各変数の重みの値を抽出
- 各変数の重みの値を元にクラスタリングを実施
- U-MAPで二次元化し可視化
